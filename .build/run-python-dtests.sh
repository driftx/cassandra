...
# WARNING (not DEBUG): the driver's reconnect loop logs full tracebacks at DEBUG, which
#  floods the Jenkins console (tens of MB per cell) and the controller's in-memory step log
pytest_opts="-vv --log-cli-level=WARNING --junit-xml=${pytest_results_file} --junit-prefix=${DTEST_TARGET} -s"
