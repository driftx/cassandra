 ...
 */
public class DirectIONativeSegment extends CommitLogSegment
{
    ByteBuffer original;
     ...
     */
    DirectIONativeSegment(CommitLog commitLog, AbstractCommitLogSegmentManager manager)
    {
	...
	original = ByteBuffer.allocateDirect(DatabaseDescriptor.getCommitLogSegmentSize() + minimumAllowedAlign);

	ByteBuffer alignedBuffer = original.alignedSlice(minimumAllowedAlign);
	assert alignedBuffer.limit() >= DatabaseDescriptor.getCommitLogSegmentSize() : String.format("Bytebuffer slicing failed to have required buffer size (required=%d,current size=%d", DatabaseDescriptor.getCommitLogSegmentSize(),alignedBuffer.limit());

        assert alignedBuffer.alignmentOffset(0, minimumAllowedAlign) ==0 : "Aligned buffer position marker must be aligned to block size";
        assert alignedBuffer.alignmentOffset(alignedBuffer.limit(), minimumAllowedAlign) ==0 : "Aligned buffer limit must be aligned to block size" ;
    ...
    protected void flush(int startMarker, int nextMarker)
    {
        ...
        try
        {
	    // lastSyncedOffset is synced to disk. Flush again from aligned marker to flush the remaining data from this block.
            long filePosition = lastSyncedOffset;
	    ByteBuffer duplicate = buffer.duplicate();

	    // Aligned file position if not aligned to block size.
	    ...
	    // Align the number of bytes to multiple of 4K.
